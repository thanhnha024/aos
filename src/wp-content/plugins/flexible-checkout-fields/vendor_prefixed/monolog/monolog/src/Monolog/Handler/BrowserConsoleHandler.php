...
class BrowserConsoleHandler extends \FcfVendor\Monolog\Handler\AbstractProcessingHandler
{
    ...
    {
        return [\FcfVendor\Monolog\Logger::DEBUG => 'debug', \FcfVendor\Monolog\Logger::INFO => 'info', \FcfVendor\Monolog\Logger::NOTICE => 'info', \FcfVendor\Monolog\Logger::WARNING => 'warn', \FcfVendor\Monolog\Logger::ERROR => 'error', \FcfVendor\Monolog\Logger::CRITICAL => 'error', \FcfVendor\Monolog\Logger::ALERT => 'error', \FcfVendor\Monolog\Logger::EMERGENCY => 'error'][$level] ?? 'log';
    }
    /**
     * @return string[]
     */
    private static function handleStyles(string $formatted) : array
    {
        ...
        foreach (\array_reverse($matches) as $match) {
            ...
            $pos = $match[0][1];
            $format = \FcfVendor\Monolog\Utils::substr($format, 0, $pos) . '%c' . $match[1][0] . '%c' . \FcfVendor\Monolog\Utils::substr($format, $pos + \strlen($match[0][0]));
        }
    ...
    private static function handleCustomStyles(string $style, string $string) : string
    {
        ...
        if (null === $style) {
            ...
            throw new \RuntimeException('Failed to run preg_replace_callback: ' . $pcreErrorCode . ' / ' . \FcfVendor\Monolog\Utils::pcreLastErrorMessage($pcreErrorCode));
        }
        return $style;
    }
    /**
