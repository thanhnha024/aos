...
class ChromePHPHandler extends \FcfVendor\Monolog\Handler\AbstractProcessingHandler
{
    ...
    protected function send() : void
    {
        ...
        $json = \FcfVendor\Monolog\Utils::jsonEncode(self::$json, \FcfVendor\Monolog\Utils::DEFAULT_JSON_FLAGS & ~\JSON_UNESCAPED_UNICODE, \true);
        $data = \base64_encode($json);
        if (\strlen($data) > 3 * 1024) {
            self::$overflowed = \true;
            $record = ['message' => 'Incomplete logs, chrome header size limit reached', 'context' => [], 'level' => \FcfVendor\Monolog\Logger::WARNING, 'level_name' => \FcfVendor\Monolog\Logger::getLevelName(\FcfVendor\Monolog\Logger::WARNING), 'channel' => 'monolog', 'datetime' => new \DateTimeImmutable(), 'extra' => []];
