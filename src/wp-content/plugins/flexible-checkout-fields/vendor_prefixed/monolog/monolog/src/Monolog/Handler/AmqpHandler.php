...
class AmqpHandler extends \FcfVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct($exchange, ?string $exchangeName = null, $level = \FcfVendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if ($exchange instanceof \FcfVendor\PhpAmqpLib\Channel\AMQPChannel) {
    ...
    private function createAmqpMessage(string $data) : \FcfVendor\PhpAmqpLib\Message\AMQPMessage
    {
        $attributes = ['delivery_mode' => 2, 'content_type' => 'application/json'];
        if ($this->extraAttributes) {
            $attributes = \array_merge($attributes, $this->extraAttributes);
        }
        return new \FcfVendor\PhpAmqpLib\Message\AMQPMessage($data, $attributes);
    }
    /**
     * {@inheritDoc}
     */
