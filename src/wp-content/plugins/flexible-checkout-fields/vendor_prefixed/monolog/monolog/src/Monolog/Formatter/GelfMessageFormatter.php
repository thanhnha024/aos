...
class GelfMessageFormatter extends \FcfVendor\Monolog\Formatter\NormalizerFormatter
{
     ...
     */
    private $logLevels = [\FcfVendor\Monolog\Logger::DEBUG => 7, \FcfVendor\Monolog\Logger::INFO => 6, \FcfVendor\Monolog\Logger::NOTICE => 5, \FcfVendor\Monolog\Logger::WARNING => 4, \FcfVendor\Monolog\Logger::ERROR => 3, \FcfVendor\Monolog\Logger::CRITICAL => 2, \FcfVendor\Monolog\Logger::ALERT => 1, \FcfVendor\Monolog\Logger::EMERGENCY => 0];
    public function __construct(?string $systemName = null, ?string $extraPrefix = null, string $contextPrefix = 'ctxt_', ?int $maxLength = null)
    {
        if (!\class_exists(\FcfVendor\Gelf\Message::class)) {
        ...
        if (\method_exists(\FcfVendor\Gelf\Message::class, 'setFacility')) {
            $this->gelfVersion = 1;
        }
    }
    /**
     * {@inheritDoc}
     */
    public function format(array $record) : \FcfVendor\Gelf\Message
    {
        $context = $extra = [];
        ...
        if ($len > $this->maxLength) {
            $message->setShortMessage(\FcfVendor\Monolog\Utils::substr($record['message'], 0, $this->maxLength));
        }
        ...
        foreach ($extra as $key => $val) {
            ...
            if ($len > $this->maxLength) {
                $message->setAdditional($this->extraPrefix . $key, \FcfVendor\Monolog\Utils::substr((string) $val, 0, $this->maxLength));
                continue;
            }
        ...
        foreach ($context as $key => $val) {
            ...
            if ($len > $this->maxLength) {
                $message->setAdditional($this->contextPrefix . $key, \FcfVendor\Monolog\Utils::substr((string) $val, 0, $this->maxLength));
                continue;
            }
