...
class NormalizerFormatter implements \FcfVendor\Monolog\Formatter\FormatterInterface
{
    ...
    /** @var int */
    private $jsonEncodeOptions = \FcfVendor\Monolog\Utils::DEFAULT_JSON_FLAGS;
    /**
    ...
    protected function normalize($data, int $depth = 0)
    {
        ...
        if (\is_object($data)) {
            ...
            return [\FcfVendor\Monolog\Utils::getClass($data) => $value];
        }
        if (\is_resource($data)) {
    ...
    protected function normalizeException(\Throwable $e, int $depth = 0)
    {
        ...
        $data = ['class' => \FcfVendor\Monolog\Utils::getClass($e), 'message' => $e->getMessage(), 'code' => (int) $e->getCode(), 'file' => $e->getFile() . ':' . $e->getLine()];
    ...
    {
        return \FcfVendor\Monolog\Utils::jsonEncode($data, $this->jsonEncodeOptions, $ignoreErrors);
    }
    /**
     * @return string
     */
    protected function formatDate(\DateTimeInterface $date)
    {
        ...
        if ($this->dateFormat === self::SIMPLE_DATE && $date instanceof \FcfVendor\Monolog\DateTimeImmutable) {
            return (string) $date;
        }
